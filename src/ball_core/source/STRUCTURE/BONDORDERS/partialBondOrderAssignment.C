...
cout << "PartialBondOrderAssignment::called estimatePenalty_()"<< endl;
		...
		cout << a_it->getFullName()<< " : " << endl;
			...
			{
				virtual_order = max((short)0,bond_orders[abop->atom_to_virtual_bond_index_[&*a_it] + abop->total_num_of_bonds_]);
			...
			if (closed)
			{
				...
				if (block < 0)
				{
					Log.error() << "Error: No penalty vector for atom " << current_atom_index << " "
								<< virtual_order << "! " << __FILE__ << " " << __LINE__ << endl;
...
cout << " ) : atom type pen = " << estimated_atom_type_penalty << ", bond len pen = "<< estimated_bond_length_penalty << endl;
																														 ...
																														 HEURISTIC_INDEX heuristic_index)
	{
		...
		float min_atom_type_penalty = numeric_limits<float>::max();
					...
					else if (heuristic_index == TIGHT)
					{
						...
						for (; b_it != atom->endBond(); b_it++)
						{
									...
									|| (bond_orders[abop->bond_to_index_[&*b_it]] <= 0)   ) // ???
							{
								...
								{
									neighbors_virtual_bonds = max((short)0,bond_orders[abop->atom_to_virtual_bond_index_[neighbor]
						...
						current_end_valence = min(current_end_valence, max_valence);
					}
					else
					{
	                                                         ...
	                                                         int fixed_virtual_order,  int fixed_valence, int num_free_bonds)
	{
		...
		// the minimal penalty of all possible penalty vectors for this atom
		float min_bond_length_penalty = numeric_limits<float>::max();
		...
		for (Size vi = fixed_virtual_order; vi < abop->atom_to_block_[atom_index].size(); vi++)
		{
			...
			if (block >= 0)
			{
				...
				if (fixed_valence <= current_end_valence)
				{
					...
					{
						float current_bond_min = numeric_limits<float>::max();
						...
						vector<float>& current_bond_length_penalties = abop->bond_lengths_penalties_[free_bonds[fb]];
						int up_to = min( (int)current_bond_length_penalties.size()-1,
