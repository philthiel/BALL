...
CHECK(operator() (AtomContainer& ac))
	...
	PDBFile infileA(TEST_DATA_PATH(ball_core/ACE_test_A.pdb));
	System sysA;
	infileA >> sysA;
	sysA.apply(bbp2);
	TEST_EQUAL(sysA.countBonds(), 1666)

	PDBFile infileB(TEST_DATA_PATH(ball_core/ACE_test_B.pdb));
	System sysB;
	infileB >> sysB;
	sysB.apply(bbp2);
	TEST_EQUAL(sysB.countBonds(), 468)
		
	SDFile infileC(TEST_DATA_PATH(ball_core/buildBondsProcessor_test.sdf));
	System sysC;
	infileC >> sysC;
