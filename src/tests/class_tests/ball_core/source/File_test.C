...
String source_name(TEST_DATA_PATH(ball_core/File_test.txt));
...
CHECK(File(const String& name, OpenMode open_mode = std::ios::in) throw(Exception::FileNotFound))
	File f(TEST_DATA_PATH(ball_core/File_test.txt));
...
CHECK(void close() throw())
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	const File& f  = file;
...
CHECK(bool open(const String& name, File::OpenMode open_mode = std::ios::in) throw(Exception::FileNotFound))
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	const File& f  = file;
	file.open(TEST_DATA_PATH(ball_core/File_test.txt));
...
CHECK(const String& getName() const throw())
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	const File& f  = file;
	TEST_EQUAL(f.getName(), TEST_DATA_PATH(ball_core/File_test.txt))
	TEST_EQUAL(file.getSize(), 100)
RESULT
...
CHECK(static Size getSize(String name) throw(Exception::FileNotFound))
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	TEST_EQUAL(file.getSize(TEST_DATA_PATH(ball_core/File_test.txt)), 100)
...
CHECK(static Type getType(String name, bool trace_link) throw(Exception::FileNotFound))
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	TEST_EQUAL(file.getType(TEST_DATA_PATH(ball_core/File_test.txt), false), 4)
	TEST_EQUAL(file.getSize(), 100)
...
CHECK(static bool copy(String source_name, String destination_name, Size buffer_size = 4096) throw(Exception::FileNotFound))
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	const File& f  = file;
	TEST_EQUAL(f.copy(TEST_DATA_PATH(ball_core/File_test.txt), TEST_DATA_PATH(ball_core/File_test.txt)), false)
	TEST_EQUAL(f.copy("", TEST_DATA_PATH(ball_core/File_test.txt)), false)
	TEST_EQUAL(f.copy(TEST_DATA_PATH(ball_core/File_test.txt), ""), false)
	TEST_EQUAL(f.copy("", ""), false)
	TEST_EXCEPTION(Exception::FileNotFound, f.copy("ZZZZZZZZZZ", "XXX"))
	TEST_EQUAL(f.copy(TEST_DATA_PATH(ball_core/File_test.txt), "XXX"), true)
	TEST_EQUAL(f.copy(TEST_DATA_PATH(ball_core/File_test.txt), "XXX"), true)
	TEST_EQUAL(file.getSize(), 100)
...
CHECK(bool copyTo(const String& destination_name, Size buffer_size = 4096) throw(Exception::FileNotFound))
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	const File& f  = file;
	TEST_EQUAL(file.copyTo(TEST_DATA_PATH(ball_core/File_test.txt)), false)
...
CHECK(static bool rename(String old_path, String new_path) throw(Exception::FileNotFound))
	File  filex(TEST_DATA_PATH(ball_core/File_test.txt));
	filex.copyTo("XXX");
...
CHECK(static bool isAccessible(String name) throw(Exception::FileNotFound))
	File  file(TEST_DATA_PATH(ball_core/File_test.txt));
	const File& f  = file;
	TEST_EQUAL(f.isAccessible(TEST_DATA_PATH(ball_core/File_test.txt)), true)
	f.remove("XXX");
...
CHECK(bool isCanonized() const throw(Exception::FileNotFound))
	File f0(TEST_DATA_PATH(ball_core/../../data/ball_core/File_test.txt));
	TEST_EQUAL(f0.isValid(), true)
	TEST_EQUAL(f0.isCanonized(), true)

	File f2(TEST_DATA_PATH(ball_core/File_test.txt));
	TEST_EQUAL(f2.isValid(), true)
	TEST_EQUAL(f2.isCanonized(), true)

	File f4(TEST_DATA_PATH(ball_core/../ball_core/File_test.txt));
	TEST_EQUAL(f4.isValid(), true)
	TEST_EQUAL(f4.isCanonized(), true)

	File f5(TEST_DATA_PATH(ball_core/.././ball_core/File_test.txt));
	TEST_EQUAL(f5.isValid(), true)
	TEST_EQUAL(f5.isCanonized(), true)

	File f6(TEST_DATA_PATH(ball_core/File_test.txt));
...
CHECK(static bool isExecutable(String name) throw(Exception::FileNotFound))
	...
	TEST_EQUAL(f.isExecutable(TEST_DATA_PATH(ball_core/File_test2.txt)), true)
#endif
...
#ifndef BALL_COMPILER_MSVC
	File f1(TEST_DATA_PATH(ball_core/File_test2.txt));
...
CHECK(std::fstream& getFileStream())
	...
	new_file.close();
	TEST_FILE(filename.c_str(), TEST_DATA_PATH(ball_core/File_test.txt))
