...
CHECK(void dump(std::ostream& s = std::cout, Size depth = 0) const throw())
  ...
  String filename;
	...
	a.dump(outfile);
	outfile.close();
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/Angle_test.txt))
RESULT

CHECK(std::istream& operator >> (std::istream& s, TAngle<T>& angle) throw())
	std::ifstream instr(TEST_DATA_PATH(ball_core/Angle_test2.txt));
	Angle a = Angle();
	instr >> a;
	instr.close();
...
CHECK(std::ostream& operator << (std::ostream& s, const TAngle<T>& angle) throw())
	...
	outstr << a;
	outstr.close();
	TEST_FILE(filename.c_str(), TEST_DATA_PATH(ball_core/Angle_test2.txt))
RESULT
