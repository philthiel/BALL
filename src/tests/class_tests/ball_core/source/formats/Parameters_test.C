...
CHECK(Parameters::Parameters(const String& filename))
  String filename(TEST_DATA_PATH(ball_core/Parameters_test.ini));
...
CHECK(Parameters::Parameters& operator = (const Parameters& parameters))
  ...
  Parameters para2;
  para2 = para;
  TEST_EQUAL(para2.getFilename(),TEST_DATA_PATH(ball_core/Parameters_test.ini))
...
CHECK(Parameters::setFilename(const String& filename) + Parameters::getFilename() const + Parameters::getParameterFile())
  Parameters para;
  para.setFilename(TEST_DATA_PATH(ball_core/Parameters_test.ini));
  TEST_EQUAL(para.getFilename(),TEST_DATA_PATH(ball_core/Parameters_test.ini))
  INIFile* inif = &para.getParameterFile();
