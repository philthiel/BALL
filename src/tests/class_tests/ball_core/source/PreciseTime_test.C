...
CHECK(PreciseTime::write(PersistenceManager& pm) const )
	...
	t.write(pm);
	of.close();
	TEST_FILE(filename.c_str(), TEST_DATA_PATH(ball_core/PreciseTime_test2.txt))
RESULT


CHECK(PreciseTime::read(PersistenceManager& pm))
	PreciseTime t;
	std::ifstream inf(TEST_DATA_PATH(ball_core/PreciseTime_test2.txt));
	pm.setIstream(inf);
	t.read(pm);
	inf.close();
...
CHECK(ostream& operator << (ostream& os, const PreciseTime& time))
	...
	of << t << std::endl;
	of.close();
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/PreciseTime_test.txt))
RESULT
