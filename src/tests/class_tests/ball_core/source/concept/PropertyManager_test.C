...
CHECK(void write(PersistenceManager& pm) const throw())
	...
	m.write(pm);
	ofile.close();	
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/PropertyManager_test_write.txt))
RESULT


CHECK(bool read(PersistenceManager& pm) throw())
	PropertyManager m;
	ifstream  ifile(TEST_DATA_PATH(ball_core/PropertyManager_test_read.txt));
	pm.setIstream(ifile);
...
CHECK(void dump(std::ostream& s = std::cout, Size depth = 0) const throw())
	...
	m.dump(outstr); 
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/PropertyManager_test_dump.txt))
RESULT
