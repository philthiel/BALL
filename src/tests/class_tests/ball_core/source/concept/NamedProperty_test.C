...
CHECK(void persistentWrite(PersistenceManager& pm, const char* name = "") const throw(Exception::GeneralException))
	...
	*np >> pm;
	ofile.close();	
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/NamedProperty_test_Float11.txt))
	delete np;
	...
	*np >> pm;
	ofile.close();	
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/NamedProperty_test_Object11.txt))
	delete np;
	...
	*np >> pm;
	ofile.close();	
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/NamedProperty_test_None11.txt))
	delete np;
	...
	*np >> pm;
	ofile.close();	
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/NamedProperty_test_String11.txt))
	delete np;
	...
	*np >> pm;
	ofile.close();	
	TEST_FILE_REGEXP(filename.c_str(), TEST_DATA_PATH(ball_core/NamedProperty_test_SmartObject11.txt))
	delete np;
RESULT
...
CHECK(void persistentRead(PersistenceManager& pm) throw(Exception::GeneralException))
	...
	ifstream  ifile(TEST_DATA_PATH(ball_core/NamedProperty_test_Float1.txt));
	pm.setIstream(ifile);
	...
	ifile.clear();
	ifile.open(TEST_DATA_PATH(ball_core/NamedProperty_test_Object1.txt));
	ptr = pm.readObject();
	ifile.close();
	...
	// due to some problems in the IRIX/CC fstream implementation....
	ifile.clear();
	ifile.open(TEST_DATA_PATH(ball_core/NamedProperty_test_None1.txt));
	ptr = pm.readObject();
	ifile.close();
	TEST_NOT_EQUAL(ptr, 0)
	if (ptr != 0)
	...
	// due to some problems in the IRIX/CC fstream implementation....
	ifile.clear();
	ifile.open(TEST_DATA_PATH(ball_core/NamedProperty_test_String1.txt));
	ptr = pm.readObject();
	ifile.close();
	...
	// due to some problems in the IRIX/CC fstream implementation....
	ifile.clear();
	ifile.open(TEST_DATA_PATH(ball_core/NamedProperty_test_SmartObject1.txt));
	ptr = pm.readObject();
	ifile.close();
