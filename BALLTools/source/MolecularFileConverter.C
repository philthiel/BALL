...
{
	CommandlineParser parpars("MolecularFileConverter", "Interconvert molecular file formats", VersionInfo::getVersion(), String(__DATE__), "Conversion");
	parpars.registerParameter("i", "Input file name", INFILE, true);
	parpars.registerParameter("if", "Input format", STRING, true);
	parpars.registerParameter("o", "Output file name", OUTFILE, true);
	...
	GenericMolFile* input  = MolFileFactory::open(parpars.get("i"), ios::in, input_format);
	GenericMolFile* output = MolFileFactory::open(parpars.get("o"), ios::out, output_format);
