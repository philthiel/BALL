...
#include <BALL_core/SCORING/FUNCTIONS/slickScore.h>
...
int main(int argc, char** argv)
{
	...
	parpars.registerParameter("rec", "input receptor file", INFILE, true);
	parpars.registerParameter("lig", "input ligand file", INFILE, true);
	parpars.registerParameter("cr", "charge rules", INFILE, false);
	parpars.registerParameter("pr", "radius rules for the polar solvation", INFILE);
	parpars.registerParameter("lj", "use FILE for LJ parameters", INFILE);
	parpars.registerParameter("op", "read options from FILE (command line overrides!)", INFILE);
	parpars.registerParameter("v", "verbosity to level", BALL::INT);
	parpars.registerParameter("s", "scaling factor for receptor charges", BALL::DOUBLE);
	parpars.registerParameter("t", "scaling factor for ligand charges", BALL::DOUBLE);
