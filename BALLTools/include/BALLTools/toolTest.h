...
\
	if (argc == 2) {\
		...
		{\
			TEST::verbose = 1;\
			TEST::keep_tool_output = true;\
		}\
	};\
\
	if ((argc > 2) || ((argc == 2) && (TEST::verbose == 0))) {\
		std::cerr << "Checks " #tool_name " class" << std::endl;\
...
\
	/* Create temporary working directory for BALL Tool */\
	Directory dir;\
	String temp_dir_name_base = "tooltest_tempdir_" #tool_name;\
\
	Size count = 1;\
	TEST::temp_dir_name = temp_dir_name_base;\
	while(dir.has(TEST::temp_dir_name))\
	{\
		TEST::temp_dir_name = temp_dir_name_base + "_" + String(count);\
		++count;\
	}\
\
	dir.create(TEST::temp_dir_name);\
	dir.set(TEST::temp_dir_name);\
	dir.setCurrent();\
\
	/* Variable necessary to iterate directory entries. */\
	String entry;\
\
	try {\
...
\
	/* Clear and remove temporary working directory */\
	while (dir.getNextEntry(entry))\
	{\
		if (!entry.hasPrefix(".") && !TEST::keep_tool_output)\
		{\
			File::remove(entry);\
		}\
	}\
\
	if (!TEST::keep_tool_output)\
	{\
		dir.remove();\
	}\
\
...
/**
 * Get the number if output files generated by the TOOL.
 * Decrement is necessary because the test log file,
 * which is not generated by the TOOL, is also located in the temporary directory.
*/
#define COUNT_OUTPUT_FILES dir.countFiles() - 1
...
/**	Opening macro for executing a TOOL.
	This Macro contains the leading code part that has to be
	terminated  using the closing Macro EXECUTE_TOOL_END.
	The macros generate code that forks a child process,
	which is used to execute a TOOL executable using passed parameters.
	The parent process waits for child process termination
	and evaluates if the latter exited successfully or not.
	The integer variable 'exit_status' is defined and a value
	of 0 indicates successful termination of the executed TOOL.
*/
#define EXECUTE_TOOL_BEGIN(toolname)\
		TEST::exit_status = 0;\
		TEST::error_message = "";\
		\
		TEST::pid = fork();\
		...
		if (TEST::pid == 0)\
		{\
			freopen("balltools_test_error.log", "w", stderr);\
			\
			execl(TOOLS_RUNTIME_OUTPUT_PATH(toolname), #toolname,

/**	Opening macro for executing a TOOL.
	This Macro contains the closing code part that has to be prefixed
	by the opening Macro EXECUTE_TOOL_Begin.
	For details see \link #EXECUTE_TOOL_BEGIN \endlink
*/
#define EXECUTE_TOOL_END()\
			, NULL);\
			\
			fclose (stderr);\
			\
		...
		{\
			...
			\
			TEST::exit_status = WEXITSTATUS(status);\
			\
			if (TEST::exit_status != 0)\
			{\
				BALL::String line;\
				std::fstream error_log("balltools_test_error.log", std::fstream::in);\
				\
				...
				{\
					...
					{\
						TEST::error_message = line;\
					}\
				}\
				\
				error_log.close();\
				BALL::File::remove("balltools_test_error.log");\
			}\
		}
