	...
	*/
	...
	{	
		...
		*/
		...
		/**
		 * Default constructor.
		 * Create a new ReconstructFragmentProcessor, which is not yet bound to a
		 * \link FragmentDB FragmentDB \endlink , i.e. which cannot yet reconstruct	fragments.
		*/
		ReconstructFragmentProcessor();
		 ...
		 * which is bound to the  \link FragmentDB FragmentDB \endlink  <tt>db</tt>.
		*/
		...
		*/
		 ...
		 * This method performs a BFS-like search for heavy atoms in the same
		 * residue the <tt>ref_center_atom</tt> is in. It will consider
		 * only atoms contained in the hash set <tt>allowed</tt>.
		 * These atoms are used as anchor points for attaching the next atom.
		*/
